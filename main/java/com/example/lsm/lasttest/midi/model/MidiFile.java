...
package com.example.lsm.lasttest.midi.model;
...
import com.example.lsm.lasttest.core.model.TimeSignature;
import com.example.lsm.lasttest.helper.ListInt;
...
public class MidiFile {
    ...
    public static ArrayList<MidiTrack> tracks ;  /** The tracks of the midifile that have notes */
    ...
    private void parse(byte[] rawdata) {
        ...
        for (MidiTrack track : tracks) {
           ...
           // Log.d("pom200", last.getNumber()+ "/");
            if (this.totalpulses < last.getStartTime() + last.getDuration()) {
                this.totalpulses = last.getStartTime() + last.getDuration();
            }

        /*
            select particle note data
            여기에 어레이 리스트를 활용해서
             넣는다
             */
        for(MidiTrack track1 : tracks){
            for(int j =0; j<track1.getNotes().size()-1;j++){
                MidiNote notedata =track1.getNotes().get(j);
                Log.d("pom200",notedata.getNumber()+"");
                //이걸로 가능하다
            }
